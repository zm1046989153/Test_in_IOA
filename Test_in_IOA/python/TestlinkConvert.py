...
class Xls2xml(object): 
    def __init__(self,path):
        ...
        self.cols_dict={'caseid':self.caseID,'keyword':self.keyword,'testname':self.testName,'path':self.path,'propertys':self.propertys,'importance':self.importance,'summary':self.summary,
                   'preconditions':self.preconditions,'stepnum':self.stepNum,'teststeps':self.teststeps,'exceptresults':self.exceptresults,'writer':self.writer,'testtype':self.testType,'teststatus':self.testStatus,'steps':self.steps}
    ...
    def excel_read(self):
        ...
        while i < rows-1:
            ...
            #else:pass
            #定义要读取的信息
            read_cols=['caseid','keyword','path','testname','propertys','importance','summary','preconditions','writer','teststatus','stepnum','teststeps','exceptresults','testtype']
